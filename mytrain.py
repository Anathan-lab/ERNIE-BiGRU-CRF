...
import time
from src.model.ernie_bigru_crf import ERNIE_BIGRU_CRF
import torch
...
def parse_arguments(parser):
    ...
    parser.add_argument('--optimizer', type=str, default="adam", help="This would be useless if you are working with transformers package")
    parser.add_argument('--learning_rate', type=float, default=1e-3)
    parser.add_argument('--momentum', type=float, default=0.0)
    parser.add_argument('--l2', type=float, default=1e-8)
    parser.add_argument('--lr_decay', type=float, default=0)
    parser.add_argument('--batch_size', type=int, default=36, help="default batch size is 10 (works well for normal neural crf), here default 36 for bert-based crf")
    parser.add_argument('--num_epochs', type=int, default=50, help="Usually we set to 50.")
    ...
    parser.add_argument('--hidden_dim', type=int, default=128, help="hidden size of the GRU usually we set to 128 for GRU-CRF")
    parser.add_argument('--dropout', type=float, default=0.5, help="dropout for embedding")

    parser.add_argument('--embedder_type', type=str, default="ernie", help="you can use 'bert-base-uncased' and so on")
...
def train_model(config: Config, epoch: int, train_loader: DataLoader, dev_loader: DataLoader, test_loader: DataLoader):
    ...
    if os.path.exists("model_files/" + model_folder):
        raise FileExistsError(
            ...
            f"to avoid override.")
    model_path = f"model_files/{model_folder}/gru_crf.m"
...
def evaluate_model(config: Config, model: ERNIE_BIGRU_CRF, data_loader: DataLoader, name: str, insts: List, print_each_type_metric: bool = False):
    ## evaluation
...
def main():
    ...
    if opt.mode == "train":
        ...
        test_dataloader = DataLoader(test_dataset, batch_size=opt.batch_size, shuffle=False, num_workers=1,
                                     collate_fn=test_dataset.collate_fn)
        model = ERNIE_BIGRU_CRF(saved_config)
        model.load_state_dict(torch.load(f"{folder_name}/gru_crf.m", map_location=device))
        model.eval()
