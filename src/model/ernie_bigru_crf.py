...
from src.model.module.bigru_encoder import BiGRUEncoder
...
class ERNIE_BIGRU_CRF(nn.Module):

    def __init__(self, config):
        super(ERNIE_BIGRU_CRF, self).__init__()
        self.transformer = TransformersEmbedder(transformer_model_name=config.embedder_type)
        if config.hidden_dim > 0:
            self.encoder = BiGRUEncoder(label_size=config.label_size, input_dim=self.transformer.get_output_dim(),
                                         hidden_dim=config.hidden_dim, drop_gru=config.dropout)
